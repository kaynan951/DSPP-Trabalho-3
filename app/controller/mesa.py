...
class MesaController:
    ...
    async def create_mesa(mesa: MesaCreate) -> Mesa:
        try:
            ...
            if response is None:
                logger.error("Erro ao criar mesa")
    ...
    async def list_mesas(
        ...
        try:
            ...
            raise HTTPException(status_code=500, detail="Erro ao listar mesas")
    ...
    async def get_mesa(mesa_id: str) -> Mesa:
        try:
            try:
                ...
                logger.warning(f"ID da mesa inválido: {mesa_id}")
                raise HTTPException(
                    status_code=400, detail="ID da mesa inválido"
                )

            if mesa is None:
                logger.warning(f"Mesa não encontrada: {mesa_id}")
    ...
    async def update_mesa(mesa_id: str, mesa_data: MesaUpdate) -> Mesa:
        try:
            ...
            if update_result.modified_count == 0:
                logger.warning(f"Nenhum campo atualizado para a mesa: {mesa_id}")
                raise HTTPException(
            ...
            logger.error(f"Erro ao atualizar mesa: {e}")
    ...
    async def delete_mesa(mesa_id: str) -> bool:
        try:
            ...
            if delete_result.deleted_count == 1:
                logger.info(f"Mesa {mesa_id} deletada com sucesso.")
                return True
            else:
                logger.warning(f"Mesa não encontrada: {mesa_id}")
                raise HTTPException(status_code=404, detail="Mesa não encontrada")
            ...
            logger.error(f"Erro ao deletar mesa: {e}")
            raise HTTPException(status_code=500, detail="Erro ao deletar mesa")
    ...
    async def num_mesa() -> Dict[str,int] :
        try:
            ...
            logger.error(f"Erro ao contar mesas: {e}")
            raise HTTPException(
    ...
    async def pegar_info_da_mesa(id: str) -> List[ComandaInfo]:
        try:
            ...
            logger.error(f"Erro ao pegar info da mesa: {e}")
            raise HTTPException(status_code=500, detail="Erro ao pegar info da mesa")